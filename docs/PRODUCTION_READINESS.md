...
1. Confirm the branch CI suite, frontend build and dependency audits are green.
2. Install locked PHP dependencies with production flags.
3. Install locked Node dependencies and build the React administration assets.
4. Run database migrations with the production confirmation flag.
5. Cache Laravel configuration, routes and views.
6. Restart queue workers and confirm the scheduler is active.
7. Verify GET /api/v1/health and GET /api/v1/health/ready.
8. Smoke-test authentication, onboarding, discovery, messaging, moderation and privacy export flows.
9. Monitor errors, latency and queue depth during rollout; use the previous release artifact for rollback.
