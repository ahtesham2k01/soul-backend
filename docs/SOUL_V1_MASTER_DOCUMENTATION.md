...
Hebrew, Punjabi and Gujarati now provide reviewed draft copy for 104 of 179 member-facing keys. Their implemented journey covers bootstrap and location messages, social and email entry, authentication failures, shared actions, primary navigation, core profile creation, photo review states, onboarding review, discovery, matching, chat presence/messaging and report/block safety actions. Hebrew remains an RTL catalog and Flutter must render it using the bootstrap direction rather than hard-coded language rules. The remaining keys continue to use safe English fallbacks until later translation packages and native-language review.
