...
- [ ] Phase 41 — Native-language review plus Android/iOS RTL, typography, truncation and device QA; all 34 draft catalogs are complete

Phases 35–39 are unambiguous engineering work. Phase 40 must not be implemented until the retention duration and legal-hold rules are approved. All Phase 41 draft catalogs are complete; only native-language and real-device visual review remains. Staging/provider gates remain tracked separately and cannot be completed from source code alone.
...
- [x] Mobile M3 — Cloudinary photo onboarding, discovery preferences, candidate cards, Like/Pass and incoming Likes, including cursor pagination, loading/empty/retry states and server-generated CDN delivery URLs for approved public photos
