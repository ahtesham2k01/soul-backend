...
- Cloudinary direct upload/moderation integration; Cloudflare-aware approximate IP location plus pluggable GPS reverse geocoding.
...
| Safety | Atomic Report/Report & Block, underage pause, privacy-minimized risk signals, suspicious-velocity and repeated-media/ban-evasion checks, moderator decisions, false-positive clearing, blocked-account appeal, chat safety nudges and immutable audit | Physical-device validation of member safety messaging remains in Mobile M9 |
...
Provider readiness is exposed to super-admin operations as safe booleans and missing variable names only. Secret values are never returned or logged. `php artisan soul:config-check --production` fails closed until every launch provider, including precise GPS reverse geocoding, is configured.
...
- [x] Phase 42 — Launch trust/abuse hardening: privacy-minimized risk signals, suspicious-velocity and ban-evasion detection, repeated-media checks, chat safety nudges, moderator evidence boundaries, false-positive recovery and regression tests

Phases 35–39 and Phase 42 are complete engineering packages. Phase 40 must not be implemented until the retention duration and legal-hold rules are approved. All Phase 41 draft catalogs are complete; server-provided RTL direction and shared selectable-control semantics are regression-guarded in Flutter. Native-language review, typography/truncation review and real-device accessibility/RTL QA remain external. Staging/provider gates remain tracked separately and cannot be completed from source code alone.

Phase 42 deliberately uses evidence already produced by normal product actions; it does not introduce device fingerprinting, raw contact harvesting or hidden identity collection. Configurable velocity monitors coalesce rapid discovery decisions and messaging into one open review case per signal. Repeating the same substantive message across several conversations creates bounded evidence containing counts, time windows and a length band—not message text. Attempting to register media already owned by another account creates a hashed repeated-media signal; reuse associated with a blocked or suspended account is labelled as possible ban evasion, never as proven evasion. Signal keys are application-keyed hashes, provider asset identifiers are excluded from moderator responses, and signals alone never ban, suspend or pause a profile. Moderators see only bounded evidence and occurrence counts, can clear false positives with a required audited reason, or can route a genuinely concerning case into the existing identity-verification flow. Flutter chat presents a dismissible neutral reminder about OTPs, passwords, money requests and off-platform pressure without accusing either participant. Thresholds are deployment-configurable through the `SOUL_SAFETY_*` variables and require staging tuning before launch.
...
The first M7 polish package closes the highest-friction entry journey without changing authentication authority. Email registration and login now provide field-specific keyboard actions and autofill hints, automatically submit a complete six-digit OTP, reject non-digit input, keep the active verification challenge isolated, and expose a 30-second client resend cooldown in addition to the server rate limit. Provider sign-in shows progress on the selected Google or Apple control while disabling competing sign-in attempts; language changes also expose progress and recover with localized configuration guidance. Auth and required-profile failures are announced as accessible live regions, text steps support keyboard submission, and the shared step scaffold dismisses the keyboard safely without obscuring its footer. These are source-level usability protections; provider sheets, large-text layout, screen readers and keyboard behavior still require the Mobile M6 physical-device matrix before launch.
...
After photos satisfy server readiness, Flutter loads the current versioned Terms, Privacy Policy, Community Guidelines and five neutral community commitments from Laravel. The member must explicitly accept them before profile submission. Submission moves through Submitted and Automated checks without exposing the profile in discovery; Flutter polls the authoritative lifecycle endpoint, renders Live, Paused or Changes required, and follows the returned `correction_screen`. Photo corrections reopen the photo journey and resubmit only after correction; profile/religion corrections return to onboarding. Existing members are also routed to a dedicated re-consent screen whenever Laravel reports a newer legal version, without repeating onboarding.
...
- [x] Complete South Asian draft expansion: Punjabi, Gujarati, Marathi, Tamil and Telugu
- [x] Complete East/Southeast Asian draft expansion: Simplified/Traditional Chinese, Japanese, Korean, Vietnamese, Thai and Filipino
- [x] Complete wider global draft expansion: Brazilian/European Portuguese, Hebrew, Ukrainian, Polish and Swahili
...
APNs, FCM, Apple Store and Google Play each have an independent shared-cache circuit breaker. Repeated transient failures open only the affected provider circuit, queued work continues through its normal retry policy without making more external calls during the cooldown, and then exactly one shared half-open probe is admitted. A successful or permanent provider response closes the circuit; another transient failure reopens it. The English-only Operations screen exposes only `open`/`half_open`/`closed`, safe failure counts and retry delay—never credentials or request data. Threshold, rolling failure window, cooldown and probe lease are configured with `SOUL_PROVIDER_CIRCUIT_*` environment values.
...
Do not treat the presence of a JSON file as proof that a translation is launch-ready. All target catalogs now have complete draft copy, but Flutter must not advertise a draft language as reviewed. Run `npm run localization:audit` to verify exact key parity and see the current draft/ready report. CI must also run `npm run localization:test`: every launch-ready catalog is rejected if it contains an unresolved English fallback, and missing or unexpected keys are rejected as drift. The audit also rejects leading/trailing whitespace, non-NFC Unicode, invisible bidirectional override/isolate controls and interpolation-placeholder drift; its JSON report exposes affected keys without leaking translated values. `config/localization-baseline.json` records the minimum coverage delivered for every target locale; removing translations or weakening that baseline fails CI. Increase a locale's baseline in the same reviewed package that adds translations, and never lower it to make a failing build pass. Monitoring and release tooling can consume `node scripts/audit-localization.mjs --format=json`; each catalog includes `localized`, `baselineLocalized`, `progress`, `fallbackKeys` and `qualityIssueKeys`. Tests run against isolated temporary catalog copies and do not change repository or runtime translations. Both GitHub workflows support manual dispatch so connector-published commits can be verified when GitHub does not emit a normal push run.

Release CI uses explicit, portable database index and foreign-key names so the complete migration chain stays within MySQL's 64-character identifier limit. `npm run migrations:audit` and `npm run migrations:test` reject any future overlong generated compound index or foreign-key identifier before PHP reaches MySQL. CI verifies a complete migration, reset and reapply cycle; rollback code restores foreign-key support indexes before removing optimized compound indexes. Redis-backed feature tests flush shared cache state between test cases so authentication and location rate limits remain deterministic under the production-like MySQL/Redis job. Privacy-export assertions follow the documented stream order without changing the versioned export schema, and provider-readiness tests require Apple notification trust configuration alongside Store API credentials. Flutter CI keeps a repository-owned widget test, enforces typed validation-field decoding and remains analyzer-safe after generating native runner projects.
...
Marathi, Tamil and Telugu now have complete draft coverage for all 179 member-facing keys, with no unresolved English fallbacks. Their coverage includes authentication, navigation, profile creation and detailed answers, photo review, onboarding, discovery, matching, likes, chat, report/block safety, notifications, privacy/account settings, private-photo protection, distance labels and legal-consent prompts. The coverage gate prevents these catalogs from regressing. All three remain drafts until native-language review and device QA are complete.

Simplified Chinese, Traditional Chinese and Japanese now have complete draft coverage for all 179 member-facing keys, with no unresolved English fallbacks. Traditional Chinese terminology is maintained separately from Simplified Chinese, and all three catalogs cover the complete member journey including privacy, safety and legal-consent copy. Their coverage baselines prevent regression; native-language review and device typography QA remain required before launch-ready status.

Korean, Vietnamese, Thai, Filipino and Swahili now also have complete draft coverage for all 179 member-facing keys, with no unresolved English fallbacks. This closes draft member-copy coverage across all 34 target catalogs while keeping the React admin English-only. The five catalogs cover authentication, onboarding, profiles, discovery, matching, likes, chat, photos, reporting and blocking, notifications, privacy and account controls, distance labels, private-photo protection and legal consent. Their 179-key coverage baselines prevent fallback regressions. They remain drafts until native-language review, truncation checks, font rendering and Android/iOS device QA are complete.
...
- Automatic recovery never requeues an event whose sensitive payload was erased after a permanent rejection. For an exhausted transient store failure, a trusted operator may run `php artisan soul:replay-store-webhook <event-id> --admin=<super-admin-email> --reason="..." --confirm=REPLAY-FAILED-STORE-WEBHOOK`. The command locks the record, accepts only a failed event with a retained encrypted payload, resets one bounded retry cycle and writes an immutable super-admin audit record. It never prints the payload.
- Run `php artisan soul:ops-check` from external monitoring at least every five minutes. It exits with code `0` when healthy and `1` when a configured queue, failure, incident-SLA or stale-work threshold is exceeded. JSON is the default; `--format=prometheus` emits scrape-safe gauges and warning labels. Both formats contain counts and stable warning codes, never job payloads, provider credentials, database names or member data. Unsupported formats fail with exit code `2`.
- The same check monitors the unprocessed store-webhook backlog plus 24-hour notification/store provider-failure totals. Configure warning levels with `SOUL_STORE_WEBHOOK_BACKLOG_WARNING_COUNT`, `SOUL_NOTIFICATION_PROVIDER_FAILURE_WARNING_COUNT` and `SOUL_STORE_PROVIDER_FAILURE_WARNING_COUNT`; keep the hard webhook admission limit safely above its warning threshold.
...
- The Operations screen also renders each provider circuit state, grouped 24-hour safe failure codes and a combined timeline of audited manual webhook recoveries plus incident acknowledgements/resolutions. Timeline entries contain only the action, responsible super-admin email, operational reason and timestamp; internal event IDs and provider payloads are excluded.
...
- Every `soul:ops-check` warning is reconciled into a durable operational incident with first/last detection, current value and threshold. The English-only Operations API and React Admin list the latest incidents; a super-admin can acknowledge or resolve an active incident from the dashboard with a mandatory operational reason. The UI reloads both incidents and audit history after each decision. Both transitions use row locking and immutable audit logs, while resolved incidents reject repeat decisions. Provider secrets and job payloads are never stored in an incident.
- Incident history retention defaults to 365 days and never removes open or acknowledged incidents. Run `php artisan soul:incident-report` or `php artisan soul:prune-incidents` for a non-destructive preview. Execution requires both `--execute` and the exact `--confirm=DELETE-EXPIRED-INCIDENTS` phrase, deletes at most 500 resolved incidents per run by default (maximum 1,000), and removes their timeline events through the database foreign key. Always review the preview and backup evidence before execution; do not schedule deletion until the owner approves the operational retention policy.
- Every incident transition is retained in a bounded Operations timeline: opened, acknowledged, manually resolved, automatically resolved after a healthy check, and reopened after recurrence. Automatic resolution has explicit system attribution and never impersonates an administrator. The dashboard defaults to active incidents and can filter active, critical or all records; the API returns the latest 50 incidents and 100 transition events.
- Critical incidents must be acknowledged within `SOUL_CRITICAL_INCIDENT_ACK_SLA_MINUTES` (15 minutes by default). Unacknowledged breaches become the machine-readable `CRITICAL_INCIDENT_ACK_SLA_BREACHED` warning, appear in Admin metrics, and request normal critical escalation. Tune this operational target only after the on-call owner and notification channel are assigned.
- Run `php artisan soul:incident-report` during capacity reviews. It reports the 30-day critical-acknowledgement sample, within-target count, average and p95 duration plus rows eligible under `SOUL_INCIDENT_HISTORY_RETENTION_DAYS` (365 days by default). This command and the matching Admin cards are strictly read-only: `deletion_performed` is always false. Actual deletion requires a separately approved legal/operations retention decision and is intentionally not automated.
- Before staging sign-off, assign `SOUL_OPERATIONS_ALERT_OWNER`, choose `SOUL_OPERATIONS_ALERT_CHANNEL=email|webhook|external`, and publish an HTTPS `SOUL_OPERATIONS_RUNBOOK_URL`. Run `php artisan soul:monitoring-readiness`; it exposes only ready/missing/invalid field names and never prints the configured owner or URL. The production configuration gate fails until this operational ownership handoff is complete. Routing delivery remains owned by the selected monitoring platform—Laravel does not send an unapproved external alert.
