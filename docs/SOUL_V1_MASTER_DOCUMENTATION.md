...
Hebrew, Punjabi and Gujarati now provide reviewed draft copy for 138 of 179 member-facing keys. Their implemented journey covers bootstrap and location messages, social and email entry, authentication failures, shared actions, primary navigation, core profile creation, photo review states, onboarding review, discovery, matching, chat, report/block safety, notifications, privacy/account settings, private-photo protection and legal-consent prompts. Hebrew remains an RTL catalog and Flutter must render it using the bootstrap direction rather than hard-coded language rules. The remaining detailed profile answers, distance labels and likes actions continue to use safe English fallbacks until the closure package and native-language review.
