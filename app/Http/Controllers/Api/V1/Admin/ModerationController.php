...
class ModerationController extends Controller
{
    ...
    {
        return ApiResponse::success(['actor' => ['id' => $r->user()->public_id, 'email' => $r->user()->email, 'role' => $r->user()->admin_role], 'counts' => ['pending_reports' => UserReport::where('status', 'pending')->count(), 'pending_verifications' => ProfileVerificationCase::whereIn('status', ['pending', 'under_review'])->count(), 'pending_appeals' => ProfileVerificationCase::where('status', 'appeal_pending')->count(), 'active_users' => User::where('status', User::STATUS_ACTIVE)->count()]]);
    }
    ...
    public function reports(): JsonResponse
    {
        ...
        return ApiResponse::success(['reports' => collect($p->items())->map(fn ($x) => ['id' => $x->public_id, 'category' => $x->category, 'details' => $x->details, 'created_at' => $x->created_at->toIso8601String()])->values(), 'next_cursor' => $p->nextCursor()?->encode()]);
    }

    public function decideReport(Request $r, string $report): JsonResponse
    {
        $v = $r->validate(['decision' => ['required', Rule::in(['resolved', 'dismissed'])], 'reason' => ['required', 'string', 'max:1000']]);
        $record = UserReport::where('public_id', $report)->first();
        if (! $record) {
            return ApiResponse::error('REPORT_NOT_FOUND', 'Report not found.', 404);
        } $this->auditUpdate($r, $record, 'report.'.$v['decision'], ['status' => $v['decision']], $v['reason']);
