...
class ModerationController extends Controller {
 public function dashboard(Request $r): JsonResponse { return ApiResponse::success(['actor'=>['email'=>$r->user()->email,'role'=>$r->user()->admin_role],'counts'=>['pending_reports'=>UserReport::where('status','pending')->count(),'pending_verifications'=>ProfileVerificationCase::whereIn('status',['pending','under_review'])->count(),'pending_appeals'=>ProfileVerificationCase::where('status','appeal_pending')->count(),'active_users'=>User::where('status',User::STATUS_ACTIVE)->count()]]); }
