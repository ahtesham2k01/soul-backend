...
class NotificationBroadcastController extends Controller
{
    ...
    public function store(Request $request): JsonResponse
    {
        ...
        if ($v['audience_type'] === 'country') {
            $v['audience_value'] = strtoupper($v['audience_value']);
        }
        $broadcast = DB::transaction(function () use ($request, $v) {
            ...
            $this->audit($request, $broadcast, 'notification_broadcast.created', null, $v, $v['reason']);
    ...
    public function send(Request $request, NotificationBroadcast $broadcast): JsonResponse
    {
        ...
        $queued = DB::transaction(function () use ($request, $broadcast, $v) {
            $locked = NotificationBroadcast::lockForUpdate()->findOrFail($broadcast->id);
            if ($locked->status !== 'draft') {
                return false;
            }
            $before = $locked->only(['status']);
            $locked->update(['status' => 'queued', 'sent_at' => now(), 'estimated_recipients' => $this->audience($locked->toArray())->count()]);
            $this->audit($request, $locked, 'notification_broadcast.queued', $before, $locked->only(['status', 'estimated_recipients']), $v['reason']);
            DeliverNotificationBroadcast::dispatch($locked->id)->afterCommit();

            return true;
        });
        if (! $queued) {
            return ApiResponse::error('BROADCAST_ALREADY_SENT', 'Only a draft broadcast can be sent.', 409);
        }
    ...
    {
        $q = User::query()->where('status', User::STATUS_ACTIVE)->whereNull('admin_role');
        $type = $broadcast['audience_type'];
        $value = $broadcast['audience_value'] ?? null;
        if ($type === 'country') {
            $q->whereHas('profile', fn ($p) => $p->where('country_code', strtoupper($value)));
        }
        if ($type === 'locale') {
            $q->where('preferred_locale', $value);
        }
        if (($broadcast['category'] ?? 'safety') === 'marketing') {
            $q->whereHas('notificationPreference', fn ($p) => $p->where('marketing', true)->orWhere('push_marketing', true)->orWhere('email_marketing', true));
        }

        return $q;
    }

    private function audit(Request $r, NotificationBroadcast $b, string $action, ?array $before, array $after, string $reason): void
    {
        AdminAuditLog::create(['admin_user_id' => $r->user()->id, 'action' => $action, 'subject_type' => NotificationBroadcast::class, 'subject_id' => $b->id, 'before' => $before, 'after' => $after, 'reason' => $reason, 'ip_address' => $r->ip()]);
    }

    private function serialize(NotificationBroadcast $x): array
    {
        return ['id' => $x->public_id, 'title' => $x->title, 'body' => $x->body, 'category' => $x->category, 'audience_type' => $x->audience_type, 'audience_value' => $x->audience_value, 'status' => $x->status, 'estimated_recipients' => $x->estimated_recipients, 'delivered_count' => $x->delivered_count, 'read_count' => $x->read_count, 'sent_at' => $x->sent_at?->toIso8601String(), 'completed_at' => $x->completed_at?->toIso8601String()];
    }
}
