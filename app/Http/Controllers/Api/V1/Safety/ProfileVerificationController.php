...
{
    public function summary(Request $request, VerificationSummary $summary): JsonResponse
    {
        return ApiResponse::success(['verification' => $summary->for($request->user())]);
    }
    ...
    public function store(Request $request): JsonResponse
    {
        ...
        if (! $hasApprovedFace) {
            return ApiResponse::error('VERIFICATION_NOT_READY', 'An approved clear-face photo is required.', 422);
        }
        ...
        $case ??= ProfileVerificationCase::query()->create([
            ...
            'requirement' => 'optional', 'status' => 'pending', 'submitted_at' => now(),
        ]);
    ...
    private function serialize(ProfileVerificationCase $case): array
    {
        return [
            ...
            'requirement' => $case->requirement, 'blocks_profile' => $case->requirement === 'required' && $case->status !== 'approved',
            'reason' => $case->reason, 'submitted_at' => $case->submitted_at->toIso8601String(),
            'reviewed_at' => $case->reviewed_at?->toIso8601String(),
            'verified_at' => $case->verified_at?->toIso8601String(),
            'appeal' => $case->appeal ? [
