...
class PrivacyController extends Controller {
 ...
 public function download(Request $r,string $export){$x=$r->user()->dataExportRequests()->where('public_id',$export)->first();$disk=Storage::disk(config('soul.privacy.export_disk'));if(!$x||$x->status!=='completed'||!$x->expires_at?->isFuture()||!$x->file_path||!$disk->exists($x->file_path))return ApiResponse::error('EXPORT_NOT_AVAILABLE','Export is not available.',404);return $disk->download($x->file_path,'soul-data-export.json',['Content-Type'=>'application/json']);}
