...
class NotificationFeedController extends Controller {
 ...
 public function read(Request $r,string $notification): JsonResponse { $n=$r->user()->notifications()->where('public_id',$notification)->first(); if(!$n)return ApiResponse::error('NOTIFICATION_NOT_FOUND','Notification not found.',404); $changed=\App\Models\UserNotification::whereKey($n->id)->whereNull('read_at')->update(['read_at'=>now()]);if($changed&&$n->broadcast_id)\App\Models\NotificationBroadcast::whereKey($n->broadcast_id)->increment('read_count');$n->refresh(); return ApiResponse::success(['id'=>$n->public_id,'read_at'=>$n->read_at->toIso8601String()]); }
}
