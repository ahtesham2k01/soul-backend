...
class MatchMessagesController extends Controller
{
    public function index(Request $request, string $match): JsonResponse
    {
        ...
        if ($conversation === null) {
            return ApiResponse::success(['messages' => [], 'next_cursor' => null]);
        }
    ...
    public function read(Request $request, string $match): JsonResponse
    {
        $record = $this->activeMatch($request, $match);
        if ($record === null) {
            return ApiResponse::error('MATCH_NOT_FOUND', 'Active match not found.', 404);
        ...
        $conversation = Conversation::query()->where('user_match_id', $record->id)->first();
        if ($conversation === null) {
            return ApiResponse::success(['marked_read' => 0, 'read_at' => now()->toIso8601String()]);
        }
        $readAt = now();
