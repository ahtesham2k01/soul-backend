...
class _AuthScreenState extends ConsumerState<AuthScreen> {
  ...
  String _title() {
    ...
    if (!widget.registration || _registrationStep == 2) return 'What’s your email address?';
    if (_registrationStep == 0) return 'What should we call you?';
    return 'What’s your DOB ?';
  }

  String _subtitle() {
    if (_challenge != null) return 'Enter the Code that we have sent to\n${_email.text.trim()}';
  ...
  Widget _fields() {
    ...
    if (_registrationStep == 0) {
      return Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          const Text('Full Name', style: TextStyle(color: SoulColors.ink, fontSize: 13)),
          const SizedBox(height: 6),
          TextField(
            controller: _name,
    ...
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        const Text('DOB', style: TextStyle(color: SoulColors.ink, fontSize: 13)),
        const SizedBox(height: 6),
        TextField(
          controller: _dob,
  ...
  Widget build(BuildContext context) {
    ...
    if (today.month < value.month || (today.month == value.month && today.day < value.day)) age--;
