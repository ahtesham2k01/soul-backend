...
type SpokenLanguage = { code:string; name:string; native_name:string; is_active:boolean; sort_order:number };
type CatalogWorkspace = { locale:string; locales:Array<{code:string;name:string;native_name:string}>; translations:Record<string,string>; overridden_keys:string[]; spoken_languages:SpokenLanguage[] };
type Operations = { social_accounts:Record<string,number>; privacy:Record<string,number>; exports:Record<string,number>; deletions:Record<string,number>; store_processing:Record<string,number>; notification_delivery:Record<string,number>; operational_health:{status:'healthy'|'warning';checked_at:string;metrics:Record<string,number>;warnings:Array<{code:string;message:string;value:number;threshold:number}>}; recent_delivery_failures:Array<{channel:string;provider:string;status:string;attempts:number;failure_code:string}>; recent_exports:Array<{id:string;user:{email:string};status:string}>; scheduled_deletions:Array<{id:string;user:{email:string};scheduled_for:string}> };
type ProfileCatalogItem = { id:string;type:'interest'|'trait';key:string;is_active:boolean;sort_order:number;translations:Record<string,string> };
type SupportTicket = { id:string;subject:string;status:string;priority:string;member:{id:string;email:string};category_key:string|null;last_message_at:string };
type DuplicateCase = { id:string;status:string;signals:Record<string,boolean>;primary:{id:string;email:string};duplicate:{id:string;email:string};merge_assessment:{safe_to_merge:boolean;blockers:string[]}|null };
...
function App() {
    ...
    const [profileCatalogs,setProfileCatalogs] = useState<ProfileCatalogItem[]>([]);
    ...
    const loadCatalogs = async () => setCatalogs((await api('/catalogs?locale='+encodeURIComponent(locale))).data);
    const loadOperations = async () => setOperations((await api('/operations')).data);
    const loadProfileCatalogs = async () => setProfileCatalogs((await api('/profile-catalogs')).data.profile_items);
    const loadSupportTickets = async () => setSupportTickets((await api('/support-tickets')).data.tickets);
    const loadDuplicateCases = async () => setDuplicateCases((await api('/duplicate-accounts')).data.cases);
    ...
    useEffect(() => { (async () => {
        ...
        await Promise.all([loadUsers(),loadAudits(),loadSupportTickets(),...(current.role==='super_admin' ? [loadAdmins(),loadReligion(),loadBroadcasts(),loadEvents(),loadEntitlements(),loadCatalogs(),loadOperations(),loadProfileCatalogs(),loadDuplicateCases()] : [])]);
    ...
    const saveTranslation = async (event:FormEvent<HTMLFormElement>) => { event.preventDefault(); const form=event.currentTarget; const data=Object.fromEntries(new FormData(form).entries()) as Record<string,string>; const reason=window.prompt(t('admin.prompt_catalog')); if(!reason)return; await api('/catalogs/translations',{method:'PUT',body:JSON.stringify({locale:data.locale,key:data.key,value:data.value,reason})}); await Promise.all([loadCatalogs(),loadAudits()]); setNotice(t('admin.notice_catalog_updated')); };
    const toggleSpokenLanguage = async (item:SpokenLanguage) => { const reason=window.prompt(t('admin.prompt_catalog')); if(!reason)return; await api('/catalogs/spoken-languages/'+item.code,{method:'PUT',body:JSON.stringify({...item,is_active:!item.is_active,reason})}); await Promise.all([loadCatalogs(),loadAudits()]); };
    const createProfileCatalogItem = async (event:FormEvent<HTMLFormElement>) => { event.preventDefault(); const form=event.currentTarget; const data=Object.fromEntries(new FormData(form).entries()) as Record<string,string>; const reason=window.prompt(t('admin.prompt_catalog')); if(!reason)return; await api('/profile-catalogs/items',{method:'POST',body:JSON.stringify({type:data.type,key:data.key,sort_order:Number(data.sort_order||0),translations:{en:data.label_en,ur:data.label_ur||data.label_en},reason})}); form.reset(); await Promise.all([loadProfileCatalogs(),loadAudits()]); };
    const toggleProfileCatalogItem = async (item:ProfileCatalogItem) => { const reason=window.prompt(t('admin.prompt_catalog')); if(!reason)return; await api('/profile-catalogs/items/'+item.id,{method:'PUT',body:JSON.stringify({is_active:!item.is_active,sort_order:item.sort_order,translations:item.translations,reason})}); await Promise.all([loadProfileCatalogs(),loadAudits()]); };
    const answerSupport = async (item:SupportTicket) => { const message=window.prompt(t('admin.support_reply')); if(!message)return; const reason=window.prompt(t('admin.prompt_decision_reason')); if(!reason)return; await api('/support-tickets/'+item.id,{method:'PUT',body:JSON.stringify({message,status:'waiting_for_member',reason})}); await Promise.all([loadSupportTickets(),loadAudits()]); };
    const resolveDuplicate = async (item:DuplicateCase,decision:'merge'|'not_duplicate') => { const reason=window.prompt(t('admin.prompt_decision_reason')); if(!reason)return; const body:Record<string,string>={decision,reason}; if(decision==='merge'){if(!window.confirm(t('admin.merge_warning')))return;body.keep_user_id=item.primary.id;body.confirmation='MERGE ACCOUNTS';} await api('/duplicate-accounts/'+item.id,{method:'PUT',body:JSON.stringify(body)}); await Promise.all([loadDuplicateCases(),loadAudits(),loadUsers()]); };
    ...
    return <div className="shell"><aside><div className="brand">SOUL</div><span>{t('admin.operations')}</span>
        ...
        <main><header id="overview"><div><small>SOUL {t('admin.operations').toUpperCase()}</small><h1>{t('admin.command_center')}</h1></div><span className="secure">● {t('admin.secure_session')}</span></header>
            ...
            {actor.role==='super_admin' && catalogs && <section className="panel" id="catalogs"><h2>{t('admin.catalogs')}</h2><div className="admin-grid"><div><h3>{t('admin.spoken_languages')}</h3>{catalogs.spoken_languages.map(item=><div className="row" key={item.code}><div><b>{item.native_name}</b><p>{item.code} · {item.name}</p></div><button className={item.is_active?'warning':''} onClick={()=>toggleSpokenLanguage(item)}>{item.is_active?t('admin.deactivate'):t('admin.activate')}</button></div>)}</div><form className="detail admin-form" onSubmit={saveTranslation}><h3>{t('admin.translation_override')}</h3><label>{t('admin.locale')}<select name="locale" defaultValue={locale}>{catalogs.locales.map(item=><option value={item.code} key={item.code}>{item.native_name}</option>)}</select></label><label>{t('admin.translation_key')}<select name="key">{Object.keys(catalogs.translations).map(key=><option key={key}>{key}</option>)}</select></label><label>{t('admin.translation_value')}<textarea name="value" required maxLength={5000}/></label><button>{t('admin.save_translation')}</button></form></div></section>}
            {actor.role==='super_admin' && operations && <section className="panel" id="operations"><h2>{t('admin.operations_health')} · {valueLabel(operations.operational_health.status)}</h2><div className="metrics">{Object.entries({...operations.operational_health.metrics,...operations.privacy,...operations.exports,...operations.deletions,...operations.store_processing,...operations.notification_delivery}).map(([key,value])=><article key={key}><strong>{value}</strong><span>{valueLabel(key)}</span></article>)}</div>{operations.operational_health.warnings.length>0&&<><h3>Operational warnings</h3>{operations.operational_health.warnings.map(item=><div className="row" key={item.code}><div><b>{item.code}</b><p>{item.message} Current {item.value}; warning threshold {item.threshold}.</p></div></div>)}</>}<h3>Store and delivery failures</h3>{operations.recent_delivery_failures.map((item,index)=><div className="row" key={`${item.channel}-${index}`}><div><b>{item.provider} · {item.channel}</b><p>{valueLabel(item.status)} · attempts {item.attempts} · {item.failure_code}</p></div></div>)}<h3>{t('admin.social_accounts')}</h3><p>{Object.entries(operations.social_accounts).map(([provider,total])=>`${provider}: ${total}`).join(' · ')||t('admin.none')}</p><h3>{t('admin.scheduled_deletions')}</h3>{operations.scheduled_deletions.map(item=><div className="row" key={item.id}><div><b>{item.user.email}</b><p>{new Date(item.scheduled_for).toLocaleString(locale)}</p></div></div>)}</section>}
            <section className="panel" id="support"><h2>{t('admin.support')}</h2>{supportTickets.length?supportTickets.map(item=><div className="row" key={item.id}><div><b>{item.subject}</b><p>{item.member.email} · {valueLabel(item.priority)} · {valueLabel(item.status)}</p></div><button onClick={()=>answerSupport(item)}>{t('admin.reply')}</button></div>):<p className="empty">{t('admin.queue_clear')}</p>}</section>
            {actor.role==='super_admin'&&<section className="panel" id="profile-catalogs"><h2>{t('admin.profile_catalogs')}</h2><div className="admin-grid"><div>{profileCatalogs.map(item=><div className="row" key={item.id}><div><b>{item.translations[locale]||item.translations.en||item.key}</b><p>{valueLabel(item.type)} · {item.key}</p></div><button className={item.is_active?'warning':''} onClick={()=>toggleProfileCatalogItem(item)}>{item.is_active?t('admin.deactivate'):t('admin.activate')}</button></div>)}</div><form className="detail admin-form" onSubmit={createProfileCatalogItem}><h3>{t('admin.create_catalog_item')}</h3><label>{t('admin.type')}<select name="type"><option value="interest">{valueLabel('interest')}</option><option value="trait">{valueLabel('trait')}</option></select></label><label>{t('admin.key')}<input name="key" required pattern="[A-Za-z0-9_-]+"/></label><label>{t('admin.english_label')}<input name="label_en" required/></label><label>{t('admin.roman_urdu_label')}<input name="label_ur"/></label><label>{t('admin.sort_order')}<input name="sort_order" type="number" min="0" defaultValue="0"/></label><button>{t('admin.create_option')}</button></form></div></section>}
            {actor.role==='super_admin'&&<section className="panel" id="duplicates"><h2>{t('admin.duplicate_accounts')}</h2>{duplicateCases.length?duplicateCases.map(item=><div className="row" key={item.id}><div><b>{item.primary.email} / {item.duplicate.email}</b><p>{valueLabel(item.status)} · {item.merge_assessment?.safe_to_merge?t('admin.safe_to_merge'):(item.merge_assessment?.blockers.join(', ')||'')}</p></div>{item.status==='open'&&<div><button disabled={!item.merge_assessment?.safe_to_merge} onClick={()=>resolveDuplicate(item,'merge')}>{t('admin.merge')}</button> <button onClick={()=>resolveDuplicate(item,'not_duplicate')}>{t('admin.not_duplicate')}</button></div>}</div>):<p className="empty">{t('admin.queue_clear')}</p>}</section>}
