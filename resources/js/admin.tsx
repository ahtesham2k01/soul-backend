...
function App() {
    ...
    return <div className="shell"><aside><div className="brand">SOUL</div><span>{t('admin.operations')}</span>
        <nav><a href="#overview">{t('admin.overview')}</a><a href="#users">{t('admin.users')}</a>{actor.role==='super_admin'&&<><a href="#admins">{t('admin.admin_access')}</a><a href="#religion">{t('admin.religion_taxonomy')}</a><a href="#broadcasts">{t('admin.broadcasts')}</a></>}<a href="#reports">{t('admin.reports')}</a><a href="#safety-cases">{t('admin.safety_cases')}</a><a href="#verification">{t('admin.verification')}</a><a href="#account-appeals">{t('admin.account_appeals')}</a><a href="#audit">{t('admin.audit_log')}</a></nav>
